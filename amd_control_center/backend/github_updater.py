"""
GitHub Update Manager for AMD Software: Adrenalin Edition.
Adapted from Cachy Security Suite and CachyOS Update Center.
...
_pkg_root = _backend_dir.parent   # .../amd_control_center
_app_root = _pkg_root.parent      # .../amd-control-center
...
def get_github_repo() -> str:
    ...
    settings = QSettings("AMD", "AMDControlCenter")
...
def get_github_token() -> Optional[str]:
    ...
    if os.path.exists(git_cred_path):
        try:
            ...
            pass
    cfg_token_path = os.path.expanduser("~/.config/amd-control-center/token")
...
class GitHubUpdateCheckerWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            headers = {
                "User-Agent": f"AMD-Control-Center/{info.installed_version}",
    ...
    def _fallback_check_tags(self, info: GitHubUpdateInfo, repo: str, token: Optional[str]) -> GitHubUpdateInfo:
        ...
        try:
            ...
            headers = {"User-Agent": f"AMD-Control-Center/{info.installed_version}"}
            if token:
                headers["Authorization"] = f"Bearer {token}"
            req = urllib.request.Request(url, headers=headers)
...
class GitHubUpdateExecWorker(QThread):
    ...
    def run(self):
        ...
        self.completed.emit(True, "AMD Software: Adrenalin Edition wurde erfolgreich auf die neueste Version aktualisiert!")
...
def download_and_install_release(version: str = "", asset_url: str = "", tarball_url: str = "", token: str = "") -> int:
    ...
    if not version or version == "Unbekannt" or (not asset_url and not tarball_url):
        try:
            ...
            headers = {"User-Agent": "AMD-Control-Center", "Accept": "application/vnd.github+json"}
            if token:
                headers["Authorization"] = f"Bearer {token}"
    ...
    print("=======================================================")
    print("  AMD Software: Adrenalin Edition - GitHub Updater    ")
    print("=======================================================")
    ...
    tmp_dir = tempfile.mkdtemp(prefix="amd_cc_update_")
    archive_path = os.path.join(tmp_dir, f"amd-control-center-v{version}.archive")
    ...
    try:
        ...
        for kind, dl_url in candidates:
            ...
            # Fallback to urllib
            headers = {"User-Agent": "AMD-Control-Center"}
            if token and ("api.github.com" in dl_url or kind == "asset_api"):
                headers["Authorization"] = f"Bearer {token}"
                if kind == "asset_api":
        ...
        if not download_ok or not os.path.exists(archive_path):
            print(f"[FEHLER] Release-Paket konnte nicht heruntergeladen werden.")
            print(f"Repository: https://github.com/{repo}")
            if token:
        ...
        res_inst = subprocess.run(["bash", installer_path], cwd=os.path.dirname(installer_path), check=False)
        ...
        print(f"✓ AMD Software: Adrenalin Edition wurde erfolgreich auf v{version} aktualisiert!")
        return 0
...
if __name__ == "__main__":
    if any(arg in sys.argv for arg in ("--download-and-install", "-h", "--help")):
        import argparse
        parser = argparse.ArgumentParser(description="AMD Control Center Standalone Release Installer")
