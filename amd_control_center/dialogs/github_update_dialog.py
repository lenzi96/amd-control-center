"""
GitHub Program Update Dialog for AMD Software: Adrenalin Edition.
Styled in native Adrenalin Crimson and dark carbon layout.
Provides automated version checking against GitHub, changelog display, and 1-click update.
"""
...
from ..backend.github_updater import (
...
class GitHubUpdateDialog(QDialog):
    """Modern modal dialog for updating AMD Software: Adrenalin Edition from GitHub."""

    app_restarted = pyqtSignal()

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Software-Aktualisierung – AMD Software: Adrenalin Edition")
        self.resize(780, 580)
    ...
    def init_ui(self):
        self.setStyleSheet(f"""
            ...
            QPushButton.primary-red:disabled {{
        ...
        title_lbl = QLabel("AMD SOFTWARE: ADRENALIN EDITION")
        title_lbl.setStyleSheet("font-size: 11px; font-weight: 900; color: #E01E37; letter-spacing: 1.5px;")
        sub_lbl = QLabel("Software- & Release-Aktualisierung (GitHub)")
        sub_lbl.setStyleSheet("font-size: 17px; font-weight: 800; color: #FFFFFF;")
        desc_lbl = QLabel("Prüfe auf neue Versionen, installiere Updates direkt per 1-Klick und verfolge Changelogs.")
        desc_lbl.setStyleSheet("font-size: 12px; color: #7E8D9F;")
        ...
        self.btn_update_now.setProperty("class", "primary-red")
        ...
        self.lbl_status.setStyleSheet("font-size: 12px; color: #7E8D9F;")
        self.lbl_status.setWordWrap(True)
        bottom_layout.addWidget(self.lbl_status, 1)

        self.btn_restart = QPushButton("🔄 Anwendung neu starten")
        self.btn_restart.setProperty("class", "primary-red")
    ...
    def _init_status_tab(self):
        ...
        icon_lbl.setStyleSheet("""
            background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 rgba(224, 30, 55, 0.25), stop:1 rgba(184, 19, 39, 0.08));
            border: 1px solid #E01E37;
            border-radius: 23px;
            color: #FF334B;
            font-size: 20px;
        ...
        lbl_head_repo.setStyleSheet("font-size: 11px; font-weight: 800; color: #E01E37; letter-spacing: 1.5px;")
        cr_layout.addWidget(lbl_head_repo)

        row_repo = QHBoxLayout()
        self.lbl_repo_name = QLabel(f"https://github.com/{get_github_repo()}")
        self.lbl_repo_name.setStyleSheet("font-family: monospace; font-size: 12px; color: #00D2FF; font-weight: 600;")
        ...
        info_lbl.setStyleSheet("font-size: 11px; color: #7E8D9F; line-height: 1.4;")
    ...
    def _init_changelog_tab(self):
        ...
        self.txt_changelog.setStyleSheet(f"""
            QTextBrowser {{
                background-color: {AdrenalinColors.BG_DARKEST};
                border: 1px solid {AdrenalinColors.BORDER_SUBTLE};
                border-radius: 6px;
                color: #C8D1DC;
                padding: 14px;
    ...
    def _on_check_finished(self, info: GitHubUpdateInfo):
        ...
        if info.has_update:
            ...
            self.lbl_status.setText(f"AMD Software: Adrenalin Edition ist auf dem neuesten Stand (v{info.installed_version}).")
        ...
        if info.release_notes:
            ...
            self.txt_changelog.setMarkdown(
                f"### AMD Software: Adrenalin Edition v{info.installed_version}\n\n"
                "- Vollständige Adrenalin-GUI für AMD Radeon Grafikkarten (RDNA 1/2/3/4)\n"
                "- FSR 4 Neural Swapper & OptiScaler Integration\n"
                "- MangoHud Overlay Editor & Gaming-Profile\n"
                "- Overclocking, Undervolting & Lüfterkurven-Steuerung\n"
                "- Schnelle Single-Instance IPC & XDG-Autostart-Integration"
            )
    ...
    def edit_github_repo(self):
        ...
        repo, ok = QInputDialog.getText(
            ...
            "(z. B. lenzi96/amd-control-center):",
            text=curr,
        )
    ...
    def run_github_update(self):
        ...
        if is_git and installer.exists():
            ...
            steps.append(
                UpdateStep(
                    name="AMD Control Center neu installieren",
                    command=["bash", str(installer)],
                    description="Kopiert Core-Dateien, aktualisiert Starter und Caches",
                )
            )
        else:
            ...
            updater_script = Path(__file__).resolve().parent.parent / "backend" / "github_updater.py"
            ...
            steps.append(
                UpdateStep(
                    name=f"AMD Software: Adrenalin Edition v{ver if ver and ver != 'Unbekannt' else 'neueste Version'} herunterladen & installieren",
                    command=cmd,
    ...
    def _on_update_completed(self, success: bool, msg: str):
        ...
        if success:
            ...
            QMessageBox.information(
                ...
                "AMD Software: Adrenalin Edition wurde erfolgreich über GitHub aktualisiert!\n\n"
